...
The public demo is intentionally synthetic. It contains no real company data, no private client data, no credentials, and no required paid API.
...
- Derives benchmark metrics in code: market share, ranks, growth, indexed values, revenue per visit, and monetization gap.
...
The public JSON stores raw monthly rows only. It does not store forecast rows, synthetic `market_total` / `market_average` rows, market-share fields, rank fields, indexed fields, or growth fields. Those are generated by the framework.
