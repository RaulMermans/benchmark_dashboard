...
The framework pipeline is the single source of truth. When `benchmark-data.json` uses `source_monthly` format:

1. Raw monthly observations are validated
2. Actual derived metrics are computed (shares, ranks, growth, efficiency, indexed)
3. Synthetic `market_total` and `market_average` benchmark rows are generated
4. Forecast rows are generated by the local engine (or TimesFM if configured)
5. Forecast rows receive the same derived-metrics pipeline as actual rows
6. The app renders from the fully computed `data.interface`

The raw JSON does **not** store forecasts or derived metrics. TimesFM is optional — the local engine runs automatically when `VITE_TIMESFM_API_URL` is not set.
